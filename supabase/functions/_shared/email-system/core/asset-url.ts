import type { EmailAssetConfiguration } from "./email-types.ts";
...
export function createEmailAssetUrl(
  ...
  if (!baseUrl) {
    throw new Error(
      "A URL-base dos assets de e-mail não foi informada.",
    );
  }

  if (!normalizedPath) {
    throw new Error(
      "O caminho do asset de e-mail não foi informado.",
    );
  }
