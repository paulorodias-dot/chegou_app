-- SISTEMA CHEGOU!
-- GATE E3.3-E.6D.2A.3
-- Acrescenta tipo_entrega autoritativo aos retornos da Entrada Oficial.
-- Não altera promoção, idempotência, eventos, logs, armazenamento, disponibilização ou notificações.
  ...
  then
    ...
    return
      ...
      jsonb_build_object(
        ...
        'tipo_entrega',
          (
            select enc.tipo_entrega
            from public.encomendas enc
            where enc.id =
                  v_operacao.encomenda_id
          )
      );
