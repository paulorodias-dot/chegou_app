...
-- - somente números: mínimo 2 caracteres
-- - demais buscas: mínimo 3 caracteres
-- - com unidade_id explícita: consulta permitida sem termo
--
-- Exemplos:
-- 05  -> permitido
-- 11  -> permitido
...
-- ANA -> permitido
--
-- Nenhuma alteração de tenant/autorização.
...
declare
  ...
  v_busca text;
  v_busca_somente_numeros boolean;
  v_busca_suficiente boolean;
...
begin
  ...
  v_busca_suficiente :=
    case
      ...
      when p_unidade_id is not null
        then true

      when v_busca is null
        then false

      when v_busca_somente_numeros
        then char_length(v_busca) >= 2

      else
        char_length(v_busca) >= 3

    end;


  if not v_busca_suficiente then
    ...
    return jsonb_build_object(
      ...
      'motivo',
        case
          when v_busca_somente_numeros
            then 'BUSCA_NUMERICA_MINIMA_2_CARACTERES'
          else
