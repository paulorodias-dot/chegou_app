...
/**
 * Sistema Chegou! — Barreira de segurança do Wizard.
 *
 * O Wizard não deve persistir imagens codificadas em Base64.
 *
 * A inclusão visual de uploads será removida das telas
 * individualmente. Esta função permanece como proteção adicional
 * contra:
 *
 * - dados legados;
 * - frontend antigo em cache;
 * - payload incompleto ou desatualizado;
 * - chamadas que ainda tragam *_base64;
 * - strings data:image/...;base64.
 *
 * Esta função NÃO converte imagem para Storage.
 */
function sanitizarPayloadSemImagemBase64(valor) {
  if (
    valor === null ||
    valor === undefined
  ) {
  ...
  if (Array.isArray(valor)) {
    return valor
      .map((item) =>
        sanitizarPayloadSemImagemBase64(
          item
        )
      )
      .filter(
...
const ETAPAS_PULADAS_SEM_MORADIA = [
  3,
  4,
];
...
const sandboxData = {
  ...
  condominio: {
    nome_fantasia:
      "Condomínio La Plaça",
    endereco:
      "Rua Exemplo, 123 • Vila Mariana • São Paulo/SP",
    sindico_nome:
      "Carlos Roberto",
    sindico_whatsapp:
      "(11) 99999-9999",
...
function normalizarPerfilUnidade(
  perfil = ""
) {
  return String(
    perfil || ""
...
function perfilDevePularEtapas(
  perfil = ""
) {
  return PERFIS_PULAM_ETAPAS.includes(
    normalizarPerfilUnidade(
      perfil
    )
  );
}

function etapaPermitidaParaPerfil(
  etapa,
  perfil = ""
) {
  if (
    perfilDevePularEtapas(
      perfil
    ) &&
    ETAPAS_PULADAS_SEM_MORADIA.includes(
      Number(
        etapa
      )
    )
...
function corrigirEtapaParaPerfil(
  etapa,
  perfil = ""
) {
  const etapaNumero =
...
function obterEtapasDesabilitadas(
  perfil = ""
) {
  return perfilDevePularEtapas(
    perfil
  )
    ? ETAPAS_PULADAS_SEM_MORADIA
    : [];
}

export default function WizardMorador({
  modoTeste = false,
}) {
  const autosaveTimer =
    useRef(
  ...
  const etapasDesabilitadas =
    obterEtapasDesabilitadas(
      perfilAtual
    );
  ...
  const chaveWelcome =
    useMemo(
      () => {
        return `wizard_morador_welcome_visto_${
          tokenAtual ||
          "sem_token"
        }`;
      },
  ...
  const inicializarFormulario =
    useCallback(
      (
        ...
        /*
         * Cadastro legado pode ainda conter Base64.
         *
         * Ele não entra no estado React do Wizard.
         */
        const dadosSeguros =
          sanitizarPayloadSemImagemBase64(
            dados
        ...
        const perfilInicial =
          pre.perfil_unidade ||
          pre.relacao_unidade ||
          dadosSeguros?.perfil_unidade ||
          dadosSeguros?.relacao_unidade ||
          "";
        ...
        const finalizadoNoBackend =
          wizardEstaFinalizado(
            dadosSeguros
          );
        ...
        setFormMorador({
          ...
          nomeSocial:
            dadosSeguros?.nome_social ||
            pre.nome_social ||
            dadosSeguros?.nomeSocial ||
            "",
      ...
      async function carregar() {
        try {
          ...
          if (
            ...
            throw new Error(
              "Token do convite não informado."
            );
          }

          const dados =
            await carregarWizardMorador(
              tokenAtual
            );

          /*
           * Primeira barreira:
           * conteúdo legado com Base64 nem entra no estado.
           */
          const dadosComToken =
            sanitizarPayloadSemImagemBase64({
              ...dados,
  ...
  function irParaEtapa(
    ...
    if (
      ...
      toast(
        "Cadastro já finalizado. As etapas anteriores estão bloqueadas."
      );
  ...
  async function salvarEResponder(
    ...
    try {
      /*
       * Terceira barreira:
       * salvar etapa nunca envia Base64.
       */
  ...
  async function concluirCadastroNaTela7(
    payloadTela7
  ) {
    const aceiteTermos =
      payloadTela7
      ...
      aceiteComunicacoes:
        Boolean(
          payloadTela7
            ?.aceite_comunicacoes ||
            payloadTela7
              ?.aceite_comunicacao_operacional
        ),
    });

    /*
     * Consolidação integral do Wizard.
     *
     * Ainda usamos os estados para compatibilidade com a
     * arquitetura atual. O objeto final passa pela sanitização
     * antes de qualquer envio ao backend.
     */
    const dadosFinais = {
      tela1:
        formTela1,
    ...
    try {
      /*
       * Quarta barreira:
       * a finalização também usa exclusivamente o payload
       * sanitizado.
       */
      const retorno =
      ...
      toast.success(
        "Cadastro concluído e enviado para análise."
      );
      ...
      console.error(
        "Erro ao concluir WizardMorador na Tela 7:",
        error
      );
    ...
    return (
      <div className="wm-loading">
        <div className="wm-loading-card">
          <h2>
            Carregando Wizard Morador...
          </h2>

          <p>
            Validando convite e preparando cadastro.
          </p>
        </div>
      </div>
    );
